...
int main()
{
	const auto parsed_mac = drc_host::MacAddress::parse("f8:54:f6:7a:5c:ae");
	...
	const auto code = drc_host::derive_code_from_ap_mac(*parsed_mac);
	...
	const auto ssid = drc_host::build_pairing_ssid(*parsed_mac, code);
	...
	const auto parsed_code = drc_host::PairingCode::from_numeric(2232);
	...
	drc_host::PairingSessionStateMachine state_machine;
	expect(state_machine.state().phase == drc_host::SessionPhase::Idle, "initial phase should be idle");
	expect(state_machine.start_pairing("wlan0", *parsed_mac, code), "start_pairing should succeed");
	expect(state_machine.state().phase == drc_host::SessionPhase::Pairing, "phase should be pairing");
	expect(state_machine.complete_pairing(), "complete_pairing should succeed");
	expect(state_machine.state().phase == drc_host::SessionPhase::Runtime, "phase should be runtime");
	...
	expect(state_machine.state().phase == drc_host::SessionPhase::Idle, "phase should return to idle");
