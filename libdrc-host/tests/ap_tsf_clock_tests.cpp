...
int main()
{
	...
	bool reset = false;
	drc_host::ApTsfClock clock([&]() -> std::optional<uint64_t> {
	...
	drc_host::ApTsfClock stopped([] { return std::optional<uint64_t>{1}; });
	expect(!stopped.update(), "inactive port value accepted");
	drc_host::ApTsfClock frozen([] { return std::optional<uint64_t>{1000}; });
	expect(frozen.update() && !frozen.update(), "nonadvancing port accepted");

	const auto wrap_origin = steady_clock::now();
	drc_host::ApTsfClock wrap([&]() -> std::optional<uint64_t> {
