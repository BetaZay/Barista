...
int main()
{
    ...
    try
    {
        ...
        auto converted = barista::api::AppHook::rgb_to_i420(black, 6, 6);
        check(converted.size() == barista::api::FrameBytes && converted[0] == 16 && converted.back() == 128, "black conversion");
        ...
        const auto logo = barista::api::AppHook::rgb_to_i420(std::vector<uint8_t>(108,100),6,6);
        ...
        barista::api::AppHook::ConnectedAppInfo lock_info{};
        check(barista::api::AppHook::read_app_lock(path, lock_info), "lock file exists while connected");
        ...
        barista::api::AppHook client2(false);
        ...
        client.stop();
        wait_for([&] { return !server.connected(); });
        check(!barista::api::AppHook::read_app_lock(path, lock_info), "lock file cleaned up on disconnect");
        ...
        check(!barista::api::AppHook::read_app_lock(path, lock_info), "stale lock auto-destroyed on read");
