#include "drh/encoder/real_replay.h"
...
int main(int argc, char** argv) {
    if (argc != 2) return 2;
    const auto replay = barista::drh::RealReplay::Load(argv[1]);
    ...
    barista::drh::MediaStreamer streamer(transport, "", true);
    ...
    for (const auto& record : replay.packets) {
        ...
        auto stamp = [&](const auto& p) -> uint32_t {
            return record.kind == 0 ? (uint32_t(p[4])<<24) | (uint32_t(p[5])<<16) |
                (uint32_t(p[6])<<8) | p[7] : barista::drh::RealReplay::LE(p.data()+pos);
        };
