#include "../src/video_packet_schedule.h"
#include <iostream>
...
int main()
{
    ...
    const auto next_format = drcd::NextFormatDeadline(origin, origin, true);
    if (next_format - origin != microseconds(11683)) return 1;
    if (next_format + drcd::kFormatVideoLead - origin != microseconds(16683)) return 1;
    ...
    if (drcd::NextFormatDeadline(origin, late, true) != late) return 1;
    if (drcd::NextFormatDeadline(origin, origin, false) - origin != microseconds(16683)) return 1;
    if (uint32_t(1000u - drcd::FormatVideoTimestamp(1000u)) != 1250u) return 1;
    if (uint32_t(6000u - drcd::FormatVideoTimestamp(1000u)) != 6250u) return 1;
    using drcd::VideoPacketOffset;
