...
class FakeBackend final : public drcd::SessionBackend
{
public:
	drcd::BackendResult start_pairing(const drcd::PairStartRequest& request) override
	{
	...
	drcd::BackendResult enter_runtime() override
	{
	...
	drcd::BackendResult start_runtime(const drcd::PairStartRequest& request) override
	{
		m_snapshot.phase = "runtime";
		m_snapshot.base_interface = request.interface_name;
		m_snapshot.ap_interface = request.interface_name;
	...
	drcd::BackendResult stop_session() override
	{
		m_snapshot = {};
		return {true, "session stopped"};
	}

	drcd::BackendSnapshot snapshot() const override
	{
		return m_snapshot;
	}
...
private:
	drcd::BackendSnapshot m_snapshot;
...
{
	{
		...
		expect(drcd::MediaStreamer::protocol_self_test(media_error),
			media_error.empty() ? "DRH media protocol self-test failed" : media_error.c_str());
	}

	drcd::ServerCore core(std::make_unique<FakeBackend>());
...

	{
		...
		drcd::ServerCore automatic_core(std::move(backend));
